...
Nesta seção, realizamos o carregamento do ambiente técnico e a unificação das bases de dados geográficos e tabulares.

```{r setup_dados, echo=TRUE, message=FALSE, warning=FALSE}
...
# Este mapa divide os preços em 5 faixas com intervalos de valores idênticos, útil para observar extremos de riqueza e pobreza de forma absoluta.
tmap_mode("plot")
...
# O mapa interativo utiliza 8 faixas baseadas em quantis, garantindo que cada cor represente um número similar de áreas, facilitando a visualização da distribuição relativa.
tmap_mode("view") 
tm_shape(shp = glasgow_dados) + 
  tm_polygons(
    fill = "preco",
...
# Definimos a vizinhança pelo critério "Queen" (áreas que se tocam em qualquer ponto). A matriz de pesos é padronizada pela linha (estilo W).
...
# Diferente do teste global, o LISA (Local Indicators of Spatial Association) identifica exatamente onde estão os agrupamentos significativos.
# Classificamos as áreas em clusters "Alto-Alto" (caros cercados por caros) e "Baixo-Baixo" (baratos cercados por baratos), além de identificar outliers espaciais.
tmap_mode("plot")
...
significantes <- lisa_local[, 5] < 0.05 # Filtro de p-valor < 0.05
...
# Visualização do Mapa de Clusters
